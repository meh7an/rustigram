...
use rustigram_types::user::{ChatId, User};
use std::time::Duration;
use tokio::sync::mpsc::{UnboundedReceiver, UnboundedSender, unbounded_channel};

// ─── Fixtures ────────────────────────────────────────────────────────────────

/// A client pointed nowhere. No test here lets a handler make a call.
fn client() -> BotClient {
...
fn command_update(command: &str) -> Update {
...
type Reports = (UnboundedSender<&'static str>, UnboundedReceiver<&'static str>);
...
fn reporting(tx: &UnboundedSender<&'static str>, name: &'static str) -> rustigram_bot::handler::BoxHandler {
    let tx = tx.clone();
...
fn failing(tx: &UnboundedSender<&'static str>, name: &'static str) -> rustigram_bot::handler::BoxHandler {
    let tx = tx.clone();
    handler_fn(move |_ctx: Context| {
        let tx = tx.clone();
        async move {
...
async fn next_report(rx: &mut UnboundedReceiver<&'static str>) -> &'static str {
...
/// running.
async fn assert_no_further_reports(rx: &mut UnboundedReceiver<&'static str>) {
